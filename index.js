...
<head>
    ...
    <title>Masry Core | تسجيل الدخول</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;500;600;700;800;900&display=swap" rel="stylesheet">
    <style>
        :root{--ink:#07111f;--panel:rgba(9,24,40,.78);--line:rgba(148,224,255,.2);--cyan:#ff304f;--violet:#8f1932;--mint:#ff8294;--muted:#9fb3c8}
        *{box-sizing:border-box}body{margin:0;min-height:100vh;overflow:hidden;font-family:'Cairo',sans-serif;color:#f5fbff;background:#08080b}
        body:before{content:"";position:fixed;inset:0;background:radial-gradient(circle at 15% 20%,rgba(255,48,79,.18),transparent 32%),radial-gradient(circle at 82% 10%,rgba(143,25,50,.2),transparent 30%),radial-gradient(circle at 70% 90%,rgba(255,130,148,.1),transparent 35%),linear-gradient(125deg,#08080b,#0a1728 55%,#08111d);z-index:-3}
        body:after{content:"";position:fixed;inset:-50%;background-image:linear-gradient(rgba(255,48,79,.045) 1px,transparent 1px),linear-gradient(90deg,rgba(255,48,79,.045) 1px,transparent 1px);background-size:54px 54px;transform:rotate(12deg);animation:drift 22s linear infinite;z-index:-2}
...
<main class="login-layout"><section class="login-panel"><div class="hero"><div class="eyebrow">MASRY CORE / CONTROL CENTER</div><div class="brand">تحكم.<br>بهيبة.</div><p>مركز موحّد لإدارة سيرفراتك، حماية مجتمعك، ومراقبة كل التفاصيل من واجهة سريعة وواضحة.</p><div class="feature-row"><span class="feature">حماية ذكية</span><span class="feature">تحليلات حيّة</span><span class="feature">صلاحيات كاملة</span></div></div><div class="login-box"><div class="eyebrow">مرحباً بعودتك</div><h1>سجّل الدخول</h1><p>اربط حساب ديسكورد للوصول إلى مركز التحكم الخاص بك.</p><a href="/auth/discord" class="login-btn"><span class="discord-mark">◉</span>المتابعة عبر Discord</a><div class="secure-note">اتصال مشفّر · صلاحياتك محفوظة · وصول آمن</div></div></section></main></body></html>`);
});
...
app.get('/manage/:guildId/bulk-role', checkAuth, async (req, res) => {
    ...
    const alertHtml = successQuery ? `
        <div style="background:rgba(0,200,83,0.15); border:1px solid #ff304f; padding:15px; border-radius:12px; margin-bottom:20px; color:#ff304f; font-weight:700;">
    ...
    const content = `
        <div class="card">
            ...
            <form method="POST" action="/save/${g.id}/bulk-role">
                ...
                <button type="submit" class="btn-save" style="font-size:16px; padding:15px; width:100%; background:linear-gradient(135deg, #ff304f, #8f1932);">🚀 بدء توزيع الرتبة على جميع الأعضاء الآن</button>
            </form>
        </div>
    `;
...
app.get('/dashboard', checkAuth, (req, res) => {
    ...
    const content = `
    <section class="card" style="margin-bottom:22px;padding:30px;background:linear-gradient(110deg,rgba(255,48,79,.12),rgba(143,25,50,.08));">
...
app.get('/manage/:guildId/home', checkAuth, async (req, res) => {
    ...
    <div class="card">
        ...
        <div class="stats-grid">
            ...
            <div class="stat-box" style="--blue:#ff304f;">
                <div class="stat-num" style="color:#ff304f;">+${newMembersCount}</div>
...
app.get('/manage/:guildId/give-role', checkAuth, async (req, res) => {
    ...
    <div class="card">
        ...
        ${message ? `<div style="background:rgba(0,200,83,0.12); border:1px solid #ff304f; color:#ff304f; padding:12px; border-radius:10px; margin-bottom:16px;">${message}</div>` : ''}
